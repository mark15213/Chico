...
- **字形级图标是重新绘制的近似版本**：蚂蚁标志（以及 ui-conversation 持有的闪光图标）使用手工几何图形，而不是从设计资源导出。
