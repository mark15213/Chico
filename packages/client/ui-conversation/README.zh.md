...
一个框架可以带来自己的开场。`conversation.hero` 以 layout 当前展示的框架为键，一个条目只替换该框架的工作区开场——标题、提示语，以及随之而来的前置条件：工作区开场在选定项目之前一直让编辑器处于禁用状态，而一个工作单元不是项目的框架没有什么要先选，因此它的空白对话可以直接输入。带自定义开场的框架旁边不会渲染工作区选择器。默认框架刻意不注册任何开场，因此在没有注册的情况下出现的就是工作区开场。
