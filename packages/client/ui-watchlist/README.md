...
Opening a name moves all three columns: the details column is revealed, and the centre column navigates to that name's newest conversation, or opens its first. Collapsing the details column keeps the open name and conversation unchanged. While a name remains open, the names frame exposes an explicit expand control that restores the column without repeating conversation navigation. The same control appears when responsive concession auto-closes a preferred-open column; its manual override lasts only until the natural layout can fit details again. Selecting or re-selecting a name also reveals it.
...
Deleting an investing conversation adds it to the registry-global archive set. It disappears from both the name's conversation list and the Sessions frame, is excluded from the Record tab's chat count, and cannot be reopened or reused as that name's newest blank conversation. If the deleted conversation was current, the workbench selects the newest remaining conversation for that name; if none remains, it clears the name selection and closes details. The same reconciliation follows archive updates from another tab or the Sessions frame while Investing is active. While another frame is active, it defers navigation until Investing returns, so an inactive frame cannot reclaim the center column. Navigation waits for the complete archive baseline after startup; a failed baseline leaves the selection retryable instead of treating an unknown set as empty. Creation rechecks the archive set after binding, and plugin disposal cancels a pending baseline wait, so neither an in-flight bind nor an unloaded workbench can reopen an archived conversation. The session log and the one-way name-record binding remain, preserving provenance and future recovery; the confirmation dialog states that this release does not provide permanent log deletion.
