...
- **The centre column is not scoped to the open name yet** — opening a name moves the left and right columns, but the conversation in the middle is whatever session is current. The host records which conversations belong to a name; nothing reads it back into the frame.
...
- **One read per mount** — neither column subscribes to quote changes, so an open workbench is as stale as its last read.
