...
The **name workbench**: the two columns Chico adds to the harness frame. It registers one navigation frame in the sidebar (`sidebar.mode`, id `names`) and one detail panel keyed to that frame (`details`). Switching to the frame swaps the left column to the followed names and the right column to the open name's record; the centre column stays the conversation, which [ui-conversation](../ui-conversation/README.md) owns.
...
Opening a name moves all three columns: the record panel is revealed, and the centre column navigates to that name's most recent conversation, or to a blank one. A conversation started under an open name is bound to it **once it stops being blank** — a blank session is the New Session view, which every name would otherwise claim in turn, while a session that has run a turn is one the user actually held about this name. A failed bind loses the association, not the conversation.
