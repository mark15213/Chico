...
import { WorkbenchFocus, type WorkbenchSessionStatus } from '../src/client/workbench-store.ts'
...
describe('the details column', () => {
  ...
  function mountColumn(
    ...
    displayName: string | null = '宁德时代',
  ) {
    const focus = new WorkbenchFocus()
    if (displayName !== null) {
      focus.open({ market: 'SZSE', symbol: '300750' }, displayName, [], sessionStatus)
    }
    ...
    return { view: render(<NameDetails {...props} />), closeDetails, focus, useSession }
  }
  ...
  it('uses the instrument code when the selected row has no display name', () => {
    const { view } = mountColumn(conversation(), 'ready', '')

    expect(view.getByRole('heading', { name: 'SZSE:300750' })).toBeTruthy()
  })

  it('shows no instrument title before a name is selected', () => {
    const { view } = mountColumn(conversation(), 'ready', null)

    expect(view.queryByRole('heading')).toBeNull()
    expect(view.queryByText('SZSE:300750')).toBeNull()
  })

  it('withholds the previous conversation while the selected name is opening', () => {
    const { view, useSession } = mountColumn(conversation(
      ask('茅台收在多少'),
      result({ name: 'market_quote', args: { market: 'SSE', symbol: '600519' } }),
    ), 'pending')

    expect(view.getByRole('status').textContent).toContain('Opening this name’s conversation')
    expect(view.queryByText('SSE:600519')).toBeNull()
    expect(useSession).not.toHaveBeenCalled()
  })

  it('keeps old evidence hidden when opening the selected conversation fails', () => {
    const { view, useSession } = mountColumn(conversation(
      ask('茅台收在多少'),
      result({ name: 'market_quote', args: { market: 'SSE', symbol: '600519' } }),
    ), 'failed')

    expect(view.getByRole('alert').textContent).toContain('Select it again to retry')
    expect(view.queryByText('SSE:600519')).toBeNull()
    expect(useSession).not.toHaveBeenCalled()
  })

  it('connects each tab to its panel and leaves only the selected tab in the tab order', () => {
    const { view } = mountColumn(conversation())
    const evidence = view.getByRole('tab', { name: 'Evidence' })
    const record = view.getByRole('tab', { name: 'Record' })

    expect(evidence.getAttribute('tabindex')).toBe('0')
    expect(record.getAttribute('tabindex')).toBe('-1')
    for (const tab of [evidence, record]) {
  ...
  it('moves focus and selection with horizontal tab keys', () => {
    const { view } = mountColumn(conversation())
    const evidence = view.getByRole('tab', { name: 'Evidence' })
    const record = view.getByRole('tab', { name: 'Record' })
    ...
    expect(record.getAttribute('tabindex')).toBe('0')
    expect(evidence.getAttribute('tabindex')).toBe('-1')
  ...
  it('keeps collapse available on both tabs without clearing the open name', () => {
    const { view, closeDetails, focus } = mountColumn(conversation())

    fireEvent.click(view.getByLabelText('Collapse investing details'))
    fireEvent.click(view.getByRole('tab', { name: 'Record' }))
    fireEvent.click(view.getByLabelText('Collapse investing details'))

    expect(closeDetails).toHaveBeenCalledTimes(2)
    expect(focus.snapshot().instrument).toEqual({ market: 'SZSE', symbol: '300750' })
  })
})
