...
每个真实 Workspace 行都带有一个**行装饰洞**（`sidebar.workspaces.rowDecoration`，`list` 类型，由 WorkspaceBrowser 条目声明）：注册方依据该行的 `workspaceId` 和已解析的 `title` 渲染尾部内容。未分组桶没有对应的 Workspace，因此不渲染该列表，注册方也不会收到没有 id 的行。装饰只负责呈现——行自身的点击仍然折叠或展开分组——并且像会话行的时间标签一样在悬停时把位置让给操作按钮，所以装饰一行既不增加宽度也不移除可用操作。空洞不渲染任何内容，行的几何保持不变。
