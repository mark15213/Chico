...
The active frame is the **layout's**, not this package's. The details column swaps occupant with it too, so a switch that only moved this column would leave the two columns describing different things. The shell forwards `detailsClosed` and `openDetails` to the active `sidebar.mode` entry, whose registrant decides when restoring that column is meaningful.

`SidebarRootComponentProps` composes the layout owner share (including frame, sidebar, and details-column viewing state), the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.mode` and `sidebar.settings` child slots, and injected `startSession`, sidebar-toggle, and the frame ledger. There is no plugin store.
