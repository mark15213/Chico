...
`sidebar.pinned` is an optional region above the browser, for content the reader keeps in view while working on something else — it does not scroll with the session list and does not change with the open session. It takes its natural height and never competes with the browser for space, so an occupant that could grow without bound caps itself. It renders only while the column is wide: the 56px rail has room for one icon per control and nothing that reads as content. No shipped composition claims it; the Chico bundle fills it with the followed-names list.

`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.pinned`, `sidebar.workspaces` and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
