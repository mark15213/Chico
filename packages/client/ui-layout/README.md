...
AppFrame always mounts the conversation and details columns; a connected Session renders through `SessionProvider`. The transient layout store starts the sidebar at its default width and details closed, and it never reads or writes `localStorage`. Hero and other unselected states also derive a zero rendered details width without changing that stored preference. AppFrame retains the last non-blank Session id across those states: the first Session remains closed, an explicit details action opens the contract default width, returning to the same Session restores its unchanged width, and selecting a different Session closes details before paint. The sidebar owner share carries `mode`, `setMode`, `collapsed`, and `width`, plus the rendered `detailsClosed` state and an `openDetails` recovery action. That action can override a responsive concession by keeping up to the details minimum and letting the centre absorb the deficit; the override is released once the natural layout can show details again. The conversation and details owner shares carry `mode`. Registrants obtain business data from standard hooks and actions from their own inject faces.
...
- **Concession-chain auto-close derives a zero width without touching the preferred width** — the panel restores itself when the window widens, while an explicit recovery temporarily keeps a usable details track; consumers must not read the stored details width as the rendered truth.
