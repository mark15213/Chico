...
每一次观测都写明它读取了哪些 Tushare 接口，以及读取结束的时刻。报价列出 `daily` 与 `stock_basic`，因为价格来自 K 线而显示名称来自名册；未复权历史只列 `daily`，复权历史列 `daily` 与 `adj_factor`。

`retrievedAt` 记的是价格读取的时间而不是名册的，后者可能已被持有长达 `rosterTtlMinutes`。价格才是读者据以行动的事实，用两者中更早的那个来标注答案会低估这个数字的新鲜度。
